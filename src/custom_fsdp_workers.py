...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == 'hf':
            ...
            print("Use VerifyvLLMRollout, Tree mode to generate rollout")
            from src.vllm_rollout_spmd import vLLMRollout
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_hf_checkpoint(self, local_path):
        """Gather FSDP shards and save a HuggingFace-format checkpoint at local_path/huggingface/."""
        assert self._is_actor
        import torch
        ...
        if torch.distributed.get_rank() == 0:
            ...
            self.actor_module_fsdp._fsdp_wrapped_module.save_pretrained(hf_local_path, state_dict=state_dict)
            self.checkpoint_manager.processing_class.save_pretrained(hf_local_path)
            print(f'Saved HF actor checkpoint to {hf_local_path}')
...
class CriticWorker(Worker):
    ...
    def save_hf_checkpoint(self, local_path):
        ...
        if torch.distributed.get_rank() == 0:
            hf_local_path = os.path.join(local_path, 'huggingface')
            os.makedirs(hf_local_path, exist_ok=True)
            self.critic_module._fsdp_wrapped_module.save_pretrained(hf_local_path, state_dict=state_dict)
            self.checkpoint_manager.processing_class.save_pretrained(hf_local_path)
            print(f'Saved HF critic checkpoint to {hf_local_path}')
